...
class RespaldoDoxBot:
    ...
    def setup_handlers(self):
        ...
        self.app.add_handler(CommandHandler("dni", self.dni_command))
        self.app.add_handler(CommandHandler("cmds", self.cmds_command))
        self.app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, self.handle_message))
        # Agregar handler para callbacks de botones
...
¡Estoy aquí para ayudarte! 🚀
    ...
    async def dni_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            # Consultar la API
            dni_data = await self.consultar_dni(dni)
            
            if dni_data:
                # Formatear respuesta
                response = self.formatear_respuesta_dni(dni_data, dni)
                
                # Si hay foto en base64, enviarla
                if dni_data.get('foto_base64'):
                    await self.enviar_foto_dni(update, context, dni_data['foto_base64'], response)
                else:
    ...
    async def consultar_dni(self, dni: str) -> dict:
        ...
        try:
            # Usar requests en un hilo separado para no bloquear el event loop
    ...
    def formatear_respuesta_dni(self, data: dict, dni: str) -> str:
...
"""
    ...
    async def enviar_foto_dni(self, update: Update, context: ContextTypes.DEFAULT_TYPE, 
